...
const DoctorStatistics = () => {
    ...
    const [selectedMonth, setSelectedMonth] = useState(''); // Empty means all months for the year
    ...
    useEffect(() => {
        const fetchStats = async () => {
            if (user && user.role === 'DOCTOR') {
                ...
                setError('Access denied. You must be a DOCTOR to view these statistics.');
                setLoading(false);
                setStats([]);
            }
        };
    ...
    return (
        <div className="statistics-container">
            <h2>Completed Examinations Referred by Center</h2>
            ...
            {stats.length === 0 ? (
                <p>No completed examination statistics found for the selected period.</p>
            ) : (
                <table className="statistics-table">
                    <thead>
                        <tr>
                            <th>Center Name</th>
                            <th>Number of Completed Examinations</th>
                        </tr>
                    </thead>
                    <tbody>
